...
1. **The gap is long-form video and audio**, not images. Claude and
   ...
   local Qwen/Gemma models powering Talia, Gus, and Kit can take a
...
2. **Retrieval-first matters because edge agents already have LLMs.**
   The OpenClaw agents run small local models with tight contexts. The
...
3. **MCP is the plug-and-play surface.** Any MCP-capable caller (Claude
   Code on Mac, OpenClaw agents on tailnet, future harnesses) gets the
...
streamable-HTTP is how Talia, Gus, Kit, and Claude Code all hit the same
...
- **Deployment is a single shared tailnet service.** One MM-RAG
  instance on Pironman (or Pi), streamable-HTTP MCP transport, all edge
  ...
  This is now live as of 2026-06-01: Pironman hosts MM-RAG at
  `http://100.126.176.86:8766/mcp`, with discovery at
  `http://100.126.176.86:8766/.well-known/mcp-resource` and the shared
  bearer token stored in `~/Projects/MM-RAG/.env` on Pironman. Production
  ...
  fix, CPU-only Docker runtime packages, atomic migrations, and Talia/Hermes
...
| M6  | `MM-RAG-xr0`  | Pi / Pironman deploy (lighter footprint)                 | P2       | scope trimmed     |
...
This is how Talia, Gus, Kit, and Claude Code all query one MM-RAG
...
### M6 — Pi / Pironman deploy (`MM-RAG-xr0`, lighter footprint)
...
models. Well within Pironman budget, comfortable on the Pi. Deploy
