...
Mac is the dev home. Pi / Pironman is the deployment floor — one
...
After any restart or redeploy, run `make check-pironman-mcp` from a shell that
...
evidence-first `ask`, and Hermes/Talia connectivity via `hermes mcp test mmrag`.
...
- **M6** **(shipped)** brings the Pi / Pironman deploy path. The image
