...
def check_mcp_health(
    ...
    hermes_server: str | None = typer.Option(
        "mmrag",
        help="Hermes MCP server name to test; pass empty string to skip.",
    ),
) -> None:
    """Run the standard post-restart Pironman MCP health check."""
    ...
    if summary.hermes_checked:
        typer.echo(f"hermes: {'ok' if summary.hermes_ok else 'failed'}")
