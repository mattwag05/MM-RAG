...
DEFAULT_PUBLIC_URL = "http://100.126.176.86:8766"
...
def run_hermes_check(server_name: str, timeout_s: float) -> bool:
    if shutil.which("hermes") is None:
        raise HealthCheckError("Hermes CLI not found on PATH.")
    ...
    if result.returncode != 0:
        ...
        raise HealthCheckError(f"Hermes MCP check failed for {server_name!r}: {output}")
    return True
